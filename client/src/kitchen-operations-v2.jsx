    ...
    <section className="operation-orders-page">
      ...
      {boardLanes.every((lane) => lane.pedidos.length === 0) ? (
        <section className="admin-panel wide">
          ...
          <p className="muted">Assim que houver pedidos, eles vao aparecer aqui por status.</p>
        </section>
      ) : null}
          ...
          <div className="kitchen-station-grid">
            {panel.estacoes.map((estacao) => (
              <section className="kitchen-station-column" key={estacao.slug}>
                ...
                <div className="kitchen-order-list">
                  {estacao.pedidos.length === 0 ? <p className="muted">Nenhum pedido para esta estacao.</p> : null}
  ...
  return (
    <article className={`kitchen-order-card${pedido.atencao ? ' attention' : ''}`}>
      ...
      <div className="kitchen-order-actions">
        ...
        {primaryAction ? (
          <button type="button" onClick={() => onOrderStatus(pedido, primaryAction.nextStatus)}>
            {primaryAction.label}
          </button>
        ) : null}
      </div>
...
function buildOrderSummary(items) {
  const totals = items.reduce(
    (accumulator, item) => {
      accumulator.items += 1;
      accumulator.units += Number(item.quantidade || 0);
      return accumulator;
    },
    { items: 0, units: 0 },
  );
  ...
  return [
    { label: 'Itens', value: totals.items, tone: 'neutral' },
    { label: 'Unidades', value: totals.units, tone: totals.units > 0 ? 'queue' : 'muted' },
  ];
}

function getOrderPrimaryAction(pedido, perfil) {
  if (pedido.status === 'Na fila' && ['admin', 'cozinha'].includes(perfil)) {
    return { label: 'Iniciar preparo', nextStatus: 'Em preparo' };
  }

  if (pedido.status === 'Em preparo' && ['admin', 'cozinha'].includes(perfil)) {
    return { label: 'Marcar como pronto', nextStatus: 'Pronto' };
  }

  if (pedido.status === 'Pronto' && ['admin', 'garcom'].includes(perfil)) {
    return { label: 'Liberar pedido', nextStatus: 'Entregue' };
  }
...
function getOrderTimeLabel(pedido, now) {
  ...
  if (pedido.status === 'Em preparo') {
    return `Em preparo ha ${formatDuration(elapsed)}`;
  }

  if (pedido.status === 'Pronto') {
    return `Pronto ha ${formatDuration(elapsed)}`;
  }

  return `Na fila ha ${formatDuration(elapsed)}`;
}
